...
class DNN_Net:
    ...
    def __init__(self, X_train, y_train, n_hidden, n_unit, n_epoch = 10, X_test = None,
        ...
        if method =='Ensemble':
            dnn_model.add(Dense(units=n_unit[0],input_dim=feature_dim,activation='relu',kernel_regularizer = regularizers.l2(0.01)))
        ...
        history = dnn_model.fit(X_train, y_train, batch_size=128, epochs=n_epoch, verbose=1, validation_split=.2)
